...
(function () {
    ...
    var migrate = function (appBucket, migrateConfig) {
        ...
        var service = {
            ...
            /**
             * Run a custom migration on a record
             *
             * @param record
             * @param documentId
             * @param customMigration
             * @param callback
             */
            runCustom: function (record, documentId, customMigration, callback) {
                return customMigration(record, function (err, newRecord) {
                    if (err) {
                        return callback(err);
                    }
             ...
             * @param {Function} custom A custom migration function that has to be called instead of the standards migrations files.
             * @param callback
             */
            batch: function (fetcher, custom, callback) {
                fetcher(function (err, docs) {
                    var tasks = lodash.map(docs, function (doc) {
                        return function (callback) {
                            ...
                            setTimeout(function () {
                                if (custom) {
                                    service.runCustom(doc.doc, doc.id, custom, function (err, record) {
                                        if (err) {
                                            return callback(err);
                                        }
